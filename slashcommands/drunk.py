...
class Drunk(apc.Group):
    ...
    async def settings(
        ...
        db = con.cursor()
        db.execute(
            "INSERT OR IGNORE INTO Alcoholist (id, weight, r, bac) VALUES (?, 80, 0.68, 0.0)",
            [ctx.user.id],
        )
        if kg != None:
    ...
    async def drink(
        ...
        db = con.cursor()
        info = db.execute(
            "SELECT weight, r FROM Alcoholist WHERE id=?", [ctx.user.id]
        ).fetchone()
        if not info:
            con.close()
            await ctx.response.send_message(
    ...
    async def howdrunk(self, ctx: discord.Interaction):
        ...
        db = con.cursor()
        bac = db.execute(
            "SELECT bac FROM Alcoholist WHERE id=?", [ctx.user.id]
        ).fetchone()
        con.close()
        if not bac:
            await ctx.response.send_message(
            ...
            return
        await ctx.response.send_message(
            f"Veresi alkoholipitoisuus on arviolta {round(bac[0], 1)} promillea.",
            ephemeral=True,
        )
