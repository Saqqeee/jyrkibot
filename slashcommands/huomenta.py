...
def populatehuomenta(db: sqlite3.Cursor, huomentalist: dict):
    ...
    for x in huomentalist["ultrarare"]:
        data.append((x, 2, 0))
    db.executemany(
        "INSERT INTO HuomentaResponses(response, rarity, rat) VALUES (?, ?, ?)", data
    )
...
class Huomenta(apc.Group):
    ...
    async def stats(
        ...
        userexists = db.execute(
            "SELECT EXISTS(SELECT * FROM Users WHERE id=?)", [user.id]
        ).fetchone()[0]
        if not userexists:
            con.close()
            ...
            return
        times = db.execute(
            "SELECT COUNT(*) FROM Huomenet WHERE uid = ?", [user.id]
        ).fetchone()
        rats = db.execute(
        ...
        morototal = db.execute(
            "SELECT COUNT(*) FROM HuomentaResponses WHERE rarity=0"
        ).fetchone()[0]
        raretotal = db.execute(
            "SELECT COUNT(*) FROM HuomentaResponses WHERE rarity=1"
        ).fetchone()[0]
        ultratotal = db.execute(
            "SELECT COUNT(*) FROM HuomentaResponses WHERE rarity=2"
        ).fetchone()[0]
        con.close()
        ...
        if times == None or times[0] < 1:
            ...
            embed.add_field(
                name="Herätykset",
                value=f"Herätty {kerrat}, joista {rats[0]} täysin rottamaiseen aikaan!",
                inline=False,
            )
    ...
    async def leaderboard(self, ctx: discord.Interaction):
        ...
        db = con.cursor()
        leaders = db.execute(
            "SELECT uid, COUNT(hour) FROM Huomenet GROUP BY uid ORDER BY COUNT(hour) DESC LIMIT 5"
        ).fetchall()
        con.close()
    ...
    async def addresponse(
        ...
        if ctx.user.id == config.owner:
            ...
            if rarity.value == 2:
                ratness.value = 0
            db.execute(
                "INSERT INTO HuomentaResponses(response, rarity, rat) VALUES (?, ?, ?)",
                [response, rarity.value, ratness.value],
            )
            con.commit()
            con.close()
