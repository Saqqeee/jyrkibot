...
async def timezone(ctx: discord.Interaction, timezones: discord.app_commands.Choice[str]):
...
class Request(apc.Group):
    ...
    async def requestadd(self, ctx: discord.Interaction, msg: apc.Range[str, 1, 100]):
        ...
        con = sqlite3.connect("data/database.db")
        db = con.cursor()
        req = db.execute("INSERT INTO Requests(uid, message, date, type) VALUES(?, ?, ?, 'new')", [ctx.user.id, msg, date])
        con.commit()
        con.close()
    ...
    async def requestlist(self, ctx: discord.Interaction, type: apc.Choice[str] = None):
        ...
        db = con.cursor()
        if not type:
            frlist = db.execute("SELECT id, uid, message, date, type FROM Requests").fetchall()
        else:
            frlist = db.execute("SELECT id, uid, message, date, type FROM Requests WHERE type=?", [type.value]).fetchall()
        con.close()
        embed = discord.Embed(
            title="Pyydetyt ominaisuudet",
            color=discord.Color.dark_magenta()
        )
        for req in frlist:
            date = datetime.fromisoformat(req[3])
            embed.add_field(name=f"**{req[0]}**, **{discord.utils.get(ctx.guild.members, id=req[1]).display_name}**: {date.strftime('%d.%m.%Y')} ({req[4]})", value=req[2], inline=False)
    ...
    async def updaterequest(self, ctx: discord.Interaction, id: int, type: apc.Choice[str]):
        if ctx.user.id == owner:
            con = sqlite3.connect("data/database.db")
            db = con.cursor()
            check = db.execute("SELECT * FROM Requests WHERE id=?", [id]).fetchone()
            if not check:
                await ctx.response.send_message("Ei ole tuollaista id:tä.", ephemeral=True)
                return
