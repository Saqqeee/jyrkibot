    ...
    if msg.content.lower() == "huomenta":
        ...
        if rarity == 2:
            userresponses = db.execute(
                "SELECT ultralist FROM HuomentaUserStats WHERE id=?", [msg.author.id]
            ).fetchone()
        elif rarity == 1:
            userresponses = db.execute(
        ...
        db.execute(
            "INSERT INTO Huomenet (uid, hour) VALUES (?, ?)", [msg.author.id, hour]
        )
        db.execute(
            "INSERT OR IGNORE INTO HuomentaUserStats(id) VALUES (?)", [msg.author.id]
        )
        if rarity == 0:
            db.execute(
